...
app.post('/signin', async (req, res) => {
  ...
  try {
    ...
    db.query(sql, [username, password], async (err, result) => {
      ...
      req.session.usernamePasswordVerified = true;
...
app.get('/check-auth', (req, res) => {
  if (req.session.user) {
    res.json({ isAuthenticated: true, usernamePasswordVerified: true });
  } else if (req.session.usernamePasswordVerified) {
    res.json({ isAuthenticated: false, usernamePasswordVerified: true });
  } else {
    res.json({ isAuthenticated: false, usernamePasswordVerified: false });
  }
});
...
app.get('/total-stock', (req, res) => {
  const sql = 'SELECT SUM(stock_quantity) as totalStock FROM products';
  ...
  db.query(sql, (err, result) => {
    ...
    res.json({ totalStock: result[0].totalStock });
  });
});
