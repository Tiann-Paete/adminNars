...
const DrawerInventory = () => {
  ...
  const fetchTotalStock = async () => {
    try {
      const response = await axios.get('http://localhost:8001/total-stock');
  ...
  const handleAddProduct = async () => {
    try {
      await axios.post('http://localhost:8001/products', currentProduct);
      fetchProducts(currentPage);
      fetchTotalStock();
    } catch (error) {
  ...
  const handleUpdateProduct = async () => {
    try {
      await axios.put(`http://localhost:8001/products/${currentProduct.id}`, currentProduct);
      fetchProducts(currentPage);
      fetchTotalStock();
    } catch (error) {
