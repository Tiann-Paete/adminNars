...
export const AuthProvider = ({ children }) => {
  ...
  const checkAuthStatus = async () => {
    try {
      const response = await axios.get('http://localhost:8001/check-auth', { withCredentials: true });
  ...
  const login = async (username, password, pin) => {
    try {
      const response = await axios.post('http://localhost:8001/signin', { username, password }, { withCredentials: true });
      setUsernamePasswordVerified(true);
      if (pin) {
        await axios.post('http://localhost:8001/validate-pin', { pin }, { withCredentials: true });
  ...
  const logout = async () => {
    try {
      await axios.get('http://localhost:8001/logout', { withCredentials: true });
      setIsAuthenticated(false);
      setUsernamePasswordVerified(false);
      router.push('/');
    } catch (error) {
